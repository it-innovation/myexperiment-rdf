<?php
	...
	$tables['policies']="policies";
        ...
        //$tables['predicate_relations']="predicate_relations";
...
PREFIX ore: <http://www.openarchives.org/ore/terms/>";
	...
	if (isset($domain) && $domain == "public"){
		...
		$sql['predicates']="select predicates.*, ontologies.uri as ontology_uri from predicates inner join ontologies on predicates.ontology_id=ontologies.id";
		//$sql['predicate_relations']="select predicate_relations.* from predicate_relations inner join predicates on predicate_relations.subject_predicate_id=predicates.id inner join ontologies on predicates.ontology_id=ontologies.id";
		...
		$sql['maps']="select maps.*, contributions.id as contribution_id, contributions.viewings_count, contributions.downloads_count, policies.id as policy_id, policies.update_mode, policies.share_mode from contributions inner join maps on contributions.contributable_id=maps.id inner join policies on contributions.policy_id=policies.id where contributable_type='Map' and ($pubcond)";
		...
		$sql['predicates']="select predicates.*, ontologies.uri as ontology_uri from predicates inner join ontologies on predicates.ontology_id=ontologies.id";
                //$sql['predicate_relations']="select * from predicate_relations";
		...
		$sql['maps']="select maps.*, contributions.id as contribution_id, contributions.viewings_count, contributions.downloads_count, policies.id as policy_id, policies.update_mode, policies.share_mode from contributions inner join maps on contributions.contributable_id=maps.id inner join policies on contributions.policy_id=policies.id where contributable_type='Map'";
		...
		$sql['pack_relationships']="select relationships.*, predicates.ontology_id from relationships inner join predicates on relationships.predicate_id=predicates.id";
	...
	$mappings['predicates']=array('id'=>'url','title'=>'rdfs:label','description'=>'rdfs:comment','ontology_uri'=>'rdfs:isDefinedBy','','created_at'=>'dcterms:created','updated_at'=>'dcterms:modified');
	...
	$nesting['predicates']=array('ontology_id');
