...
function getResidence($user){
	...
	if(isset($user['location_city']) && strlen(trim($user['location_city']))>0) {
		$city = str_replace("+","_",urlencode(ucwords(str_replace($mats,$reps,$user['location_city']))));
	...
	if(isset($user['location_country']) && strlen(trim($user['location_country']))>0) {
		$country = str_replace("+","_",urlencode(ucwords(str_replace($mats,$reps,$user['location_country']))));
