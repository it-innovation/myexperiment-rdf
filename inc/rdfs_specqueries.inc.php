...
select distinct ?class ?property where { GRAPH <$reasonedont> { ?class rdf:type rdfs:Class . ?class rdfs:subClassOf ?sclass . ?property rdfs:domain ?sclass . FILTER(REGEX(str(?class),'^$filteront') && REGEX(str(?sclass),'^$filteront'))}}";
...
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
select distinct ?class ?label ?comment where { GRAPH <$reasonedont> { ?class rdf:type rdfs:Class . OPTIONAL{?class rdfs:label ?label} . OPTIONAL{?class rdfs:comment ?comment} . FILTER(REGEX(str(?class),'^$filteront'))} }";
...
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
select distinct ?myclass ?exclass where { GRAPH <$reasonedont> { ?myclass rdf:type rdfs:Class . ?myclass rdfs:subClassOf ?exclass . FILTER( !REGEX(STR(?exclass),'^$filteront') && !REGEX(STR(?exclass),'^http://www.w3.org/2000/01/rdf-schema#Resource') && REGEX(str(?myclass),'^$filteront'))}}";

//Query 9: SubProperty Properties
$queries[9]="PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
select distinct ?myprop ?exprop where { GRAPH <$reasonedont> { ?myprop rdf:type rdf:Property . ?myprop rdfs:isDefinedBy <$filteront> . ?myprop rdfs:subPropertyOf ?exprop . FILTER( !REGEX(STR(?exprop),'^$filteront') && REGEX(str(?myprop),'^$filteront')) }}";
