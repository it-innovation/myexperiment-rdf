...
if [ `cat /tmp/dataflows.txt | wc -l` -gt 0 ]; then
	...
	java -cp $JAVA_CP RDFSDataReasoner $STORE4_PATH/config/${TRIPLESTORE}_ontologies.txt /tmp/dataflows.txt $DATA_PATH/dataflows/reasoned/
	echo "java -cp $JAVA_CP RDFSDataReasoner $STORE4_PATH/config/${TRIPLESTORE}_ontologies.txt /tmp/dataflows.txt $DATA_PATH/dataflows/reasoned/"
