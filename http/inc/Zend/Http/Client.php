...
class Zend_Http_Client
{
     ...
     */
    public function request($method = null, $bodypar = '', $acceptpar = '', $ctpar = '', $addpar='')
    {
	    ...
	    if ($method=="PUT" || $method=="POST") $body = $this->prepare_body($bodypar);
	    else $body = $this->prepare_body();
            if ($acceptpar||$ctpar||$addpar) $headers = $this->prepare_headers($acceptpar,$ctpar,$addpar);
	    else $headers = $this->prepare_headers();
		...
		//print_r($this->last_request);
            ...
            if (! $response) {
                require_once 'Zend/Http/Client/Exception.php';
		for ($h=0; $h<sizeof($headers); $h++) $headtxt.=$headers[$h]."\n";
     ...
     * @return array
     */
    protected function prepare_headers($headeraccept=null, $headerct=null, $headeradd=null)
    {
        $headers = array();
	...
	if ($headeradd){
		$hbits=explode('\n',$headeradd);
		for ($h=0; $h<sizeof($hbits); $h++){
			$curhead=trim($hbits[$h]);
			if ($curhead) $headers[] = $curhead;
		}
	}
        return $headers;
    }
     ...
     * @return string
     */
    protected function prepare_body($bodypar = '')
    {
